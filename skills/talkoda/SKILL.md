...
**Story length:** `story.md` is uploaded as the API's `summary`. Default to **1200 characters or fewer**, with a **hard maximum of 2000**. This is not a word count: count Unicode code points after trimming (`Array.from(text.trim()).length`), including punctuation, internal spaces and line breaks; combined emoji may contain several code points. Keep the motif, meaningful turns and musical mapping, and remove padding. A longer local essay can be kept in a separate file; upload only the short public story.

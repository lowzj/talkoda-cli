...
export function apiOrigin(value = 'https://talkoda.com') {
  ...
  try {
    ...
    throw new Error('API 地址必须是完整的 HTTPS 域名。')
  }
  if (url.username || url.password || url.search || url.hash || url.pathname !== '/') {
    throw new Error('API 地址只填写域名，不包含账号、路径、参数或片段。')
  }
  if (
    url.protocol !== 'https:' &&
    !(url.protocol === 'http:' && ['127.0.0.1', 'localhost', '[::1]'].includes(url.hostname))
  ) {
    throw new Error('API Token 只通过 HTTPS 发送；HTTP 仅允许本地回环地址。')
  }
  return url.origin
}
...
export function validateToken(token) {
  if (!/^tk_[a-f0-9]{64}$/.test(token))
    throw new Error('Token 格式不正确，请在网站的 API Token 页面创建。')
  return token
}
...
export async function readConfig() {
  try {
    ...
    throw new Error('CLI 配置文件不可读取或格式无效，请检查 TALKODA_CONFIG_FILE。', {
      cause: error,
    })
  }
}
...
export function client(origin, token) {
  async function request(path, { method = 'GET', body, headers = {}, raw = false } = {}) {
    ...
    if (!response.ok) {
      ...
      const message = typeof data?.error === 'string' ? data.error : 'API 请求失败'
      throw new Error(
    ...
    const data = await response.json().catch(() => null)
    if (!data || typeof data !== 'object') throw new Error('API 返回了无法识别的响应。')
    return data
  }
  return {
    request,
    requireToken() {
      if (!token) throw new Error('请先运行 talkoda auth login，或设置 TALKODA_API_TOKEN。')
    },
  }
}

...
/** Match the API's z.string().trim().max(2000): Zod counts Unicode code points. */
...
export function storyLengthError(characters, field = 'story') {
  return new Error(
    t(
      `故事简介（${field}）有 ${characters} 字符，最多 ${STORY_MAX_CHARACTERS}（去掉首尾空白后按 Unicode 码点计数）。建议压缩到 ${STORY_TARGET_CHARACTERS} 字符以内再重试；未发送 API 请求。`,
      `Story (${field}) has ${characters} characters; maximum ${STORY_MAX_CHARACTERS} (Unicode code points after trimming). Aim for ${STORY_TARGET_CHARACTERS} or fewer before retrying. No API request was sent.`,
    ),
  )
}

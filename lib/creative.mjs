...
export async function creativeCommand(args, flags) {
  ...
  if (args[0] === 'compose') {
    check(2, ['conversation', 'output', 'title', 'bpm', 'cycles', ...generationOptions])
    if (args[1] !== 'init' || !flags.conversation || !flags.output)
      throw new Error(
        t(
          '使用 compose init --conversation FILE --output DIRECTORY。',
          'Use compose init --conversation FILE --output DIRECTORY.',
        ),
      )

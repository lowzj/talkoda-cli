...
async function browserPath(explicit) {
  ...
  const candidates = [
    // Ubuntu's AppArmor profile permits the system Chrome binary to use its sandbox.
...
export async function rendererStatus(explicit) {
  ...
  return {
    ...
    ...(executable
      ...
      : {
          next: t(
            '运行 talkoda render setup 安装 Chromium。Linux 还需满足 Playwright 浏览器系统依赖。',
            'Run talkoda render setup to install Chromium. Linux also requires the Playwright browser system dependencies.',
          ),
        }),
  }
}
    ...
    child.on('close', (code) =>
      code === 0
        ...
        : reject(
            new Error(
              t(
                'Chromium 安装失败，请检查网络和 Playwright 系统依赖。',
                'Chromium installation failed. Check your network and Playwright system dependencies.',
              ),
            ),
          ),
    )
...
function pcmFromWav(bytes) {
  if (bytes.toString('ascii', 0, 4) !== 'RIFF' || bytes.toString('ascii', 8, 12) !== 'WAVE')
    throw new Error(t('Strudel 未返回有效 WAV。', 'Strudel did not return a valid WAV.'))
  let format, data
  for (let offset = 12; offset + 8 <= bytes.length;) {
    const name = bytes.toString('ascii', offset, offset + 4),
      size = bytes.readUInt32LE(offset + 4),
      begin = offset + 8
    if (begin + size > bytes.length)
      throw new Error(t('WAV 数据不完整。', 'WAV data is incomplete.'))
    if (name === 'fmt ' && size >= 16)
      format = {
  ...
  if (
    ...
    throw new Error(
      t(
        '渲染结果必须是 48 kHz 双声道 16-bit PCM。',
        'Render output must be 48 kHz stereo 16-bit PCM.',
      ),
    )
...
export async function renderAudio(flags) {
  if (!flags.source || !flags.output || !flags.bpm || !flags.cycles)
    throw new Error(
      t(
        '使用 render --source FILE --output FILE.mp3 --bpm N --cycles N。',
        'Use render --source FILE --output FILE.mp3 --bpm N --cycles N.',
      ),
    )
  ...
  if (
    ...
    throw new Error(
      t(
        '渲染要求 20–300 BPM、正整数 cycles，4/4 总时长不超过 5 分钟。',
        'Rendering requires 20–300 BPM, positive integer cycles and a 4/4 duration no longer than 5 minutes.',
      ),
    )
  const sourcePath = resolve(flags.source),
    output = resolve(flags.output)
  if (extname(output).toLowerCase() !== '.mp3')
    throw new Error(t('渲染输出使用 .mp3 扩展名。', 'Use the .mp3 extension for render output.'))
  const sourceInfo = await stat(sourcePath)
  if (!sourceInfo.isFile() || !sourceInfo.size || sourceInfo.size > 128 * 1024)
    throw new Error(
      t(
        'Strudel 源码需为 1 B–128 KB 的文本文件。',
        'Strudel source must be a text file of 1 B–128 KB.',
      ),
    )
  ...
  if (!executable)
    throw new Error(
      t(
        '未找到 Chrome/Chromium，请先运行 talkoda render setup。',
        'Chrome/Chromium was not found. Run talkoda render setup first.',
      ),
    )
  ...
  try {
    ...
    temporary = await mkdtemp(join(cache, 'render-'))
    ...
    const environment = Object.fromEntries(
      ['PATH', 'SystemRoot', 'WINDIR', 'TEMP', 'TMP', 'TMPDIR', 'LANG', 'CHROME_DEVEL_SANDBOX']
    ...
    try {
      ...
      if (/sandbox/i.test(error.message))
        throw new Error(
          t(
            '浏览器沙箱不可用。Linux 请优先使用系统安装的 Google Chrome（--browser /opt/google/chrome/chrome），或配置系统支持的 Chromium 沙箱。',
            'The browser sandbox is unavailable. On Linux use system Google Chrome (--browser /opt/google/chrome/chrome), or configure a supported Chromium sandbox.',
          ),
          { cause: error },
        )
      throw error
    ...
    const work = page.evaluate(
      async ({ code, bpm, cycles, language }) => {
        ...
        if (!pattern)
          throw new Error(
            t(`Strudel 编译失败：${compileError}`, `Strudel compilation failed: ${compileError}`),
          )
        if (Math.abs(repl.scheduler.cps - bpm / 240) > 0.000001)
          throw new Error(
            t(
              '源码的 setcps/setcpm 与 --bpm 不一致，请同步修改。',
              'The source setcps/setcpm does not match --bpm. Update them together.',
            ),
          )
        ...
        if (!events.length || events.length > 50000)
          throw new Error(
            t('作品必须包含 1–50000 个音符事件。', 'A track must contain 1–50000 note events.'),
          )
        const sounds = [
          ...new Set(
            events.map((event) => {
        ...
        if (sounds.some((sound) => !supported.includes(sound)))
          throw new Error(
            t(
              ...
              `Offline rendering supports built-in synthesizers only. Replace these sounds: ${sounds.filter((sound) => !supported.includes(sound)).join(', ')}`,
            ),
          )
        await engine.renderPatternAudio(pattern, bpm / 240, 0, cycles, 48000, 512, false, 'talkoda')
        return { events: events.length, sounds }
      },
      { code: source, bpm, cycles, language: getLanguage() },
    )
        ...
        timer = setTimeout(
          () =>
            reject(
              new Error(
                t(
                  '渲染超过 120 秒，已停止。请简化声部或缩短编曲。',
                  'Rendering exceeded 120 seconds and was stopped. Simplify the parts or shorten the arrangement.',
                ),
              ),
            ),
          120000,
    ...
    if (await file.failure()) throw new Error(t('WAV 导出失败。', 'WAV export failed.'))
    const wavPath = join(temporary, 'render.wav')
    await file.saveAs(wavPath)
    if (blockedRequests)
      throw new Error(
        t(
          '源码尝试访问网络；离线渲染仅使用内置合成器，请移除外部资源。',
          'The source attempted network access. Offline rendering uses built-in synthesizers only; remove external resources.',
        ),
      )
    if (problems.length)
      throw new Error(
        t(
          `Strudel 渲染存在错误：${problems[0].slice(0, 250)}`,
          `Strudel rendering failed: ${problems[0].slice(0, 250)}`,
        ),
      )
    if ((await stat(wavPath)).size > 60 * 1024 * 1024)
      throw new Error(t('渲染文件超出限制。', 'The rendered file exceeds the size limit.'))
    const pcm = pcmFromWav(await readFile(wavPath))
    if (Math.abs(pcm.frames / pcm.sampleRate - seconds) > 0.05)
      throw new Error(
        t('渲染时长与计划不匹配。', 'The rendered duration does not match the planned duration.'),
      )
    let peak = 0,
      energy = 0
    ...
    if (peak < 0.0001)
      throw new Error(
        t(
          '渲染结果接近静音，请检查音符和声音名称。',
          'The render is nearly silent. Check notes and sound names.',
        ),
      )
    if (peak >= 0.999)
      throw new Error(
        t(
          '合成音频接近满幅，请降低各声部 gain 后重新渲染。',
          'Synthesized audio is near full scale. Lower the gain of each part and render again.',
        ),
      )
    ...
    if (mp3.length > 16 * 1024 * 1024)
      throw new Error(
        t('MP3 超出 Talkoda 的 16 MB 上限。', 'The MP3 exceeds the Talkoda 16 MB limit.'),
      )
